...
class HashMultiset<E> implements Multiset<E> {
    ...
    public void union(Multiset<E> other) {
        Set<E> otherElem = other.toSet();
        for (E elem : otherElem) {
            var count = other.getMultiplicity(elem) - this.getMultiplicity(elem);
            if (count > 0) {
    ...
    public void intersect(Multiset<E> other) {
        ...
        for (E elem : keySet) {
            if (!other.contains(elem)) {
                ...
                var count = this.getMultiplicity(elem) - other.getMultiplicity(elem);
                if (count > 0) {
                    for (int i = 0; i < count; i++) {
                        this.remove(elem);
                    }
    ...
    public int size() {
        sizeElements = 0;
        map.forEach((key, value) -> sizeElements += value);
        return sizeElements;
    }
